  ...
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
      ...
      self.names = coco_class_name
        ...
        self.names=three_class_name
    elif dataset == 'mydata':
        self.names = mydata_class_name
    elif dataset == 'FullData':
        self.names = FullData_class_name
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    if show_txt:
      cv2.rectangle(self.imgs[img_id],
                    ...
                    (bbox[0] + cat_size[0]+15, bbox[1] - 10), c, -1)
      cv2.putText(self.imgs[img_id], txt, (bbox[0], bbox[1] +10- 2),
                  font, 0.6, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
  ...
  def show_all_imgs(self, pause=False, time=0):
    if not self.ipynb:
      for i, v in self.imgs.items():
        #cv2.namedWindow(came , cv2.WINDOW_AUTOSIZE)
        #cv2.imshow(came , v)
...
FullData_class_name = ['person','fire','knife']
